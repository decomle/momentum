export {default as useRegister} from '@/hooks/auth/useRegister'
export {default as useLogin} from '@/hooks/auth/useLogin'
export {default as useLogout} from '@/hooks/auth/useLogout'
export {default as useDashboard} from '@/hooks/useDashboard'
export {default as useUpdateProfile} from '@/hooks/auth/useUpdateProfile'
export {default as useHabit} from '@/hooks/habit/useHabit'
export {default as useHabitSummary} from '@/hooks/habit/useHabitSummary'
export {default as useLogHabit} from '@/hooks/habit/useLogHabit'

export {default as useScrollPosition} from '@/hooks/useScrollToPosition'
export { useAuth } from "@/contexts/AuthContext"
export { useUser } from "@/contexts/UserContext"
